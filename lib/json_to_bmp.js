...
exports.JSONtoBmp = function (bmpData, byteOps) {
    ...
    if (bmpData.bitsPerPixel === 32) {
        pixelMapToBuffer(rawBmp, bmpData, writeBuffer.writeColor32);
    } else if (bmpData.bitsPerPixel === 24) {
        pixelMapToBuffer(rawBmp, bmpData, writeBuffer.writeColor24);
    } else if (bmpData.bitsPerPixel === 8) {
        writePalette(rawBmp, bmpData, writeBuffer.writeColor32);
        pixelMapToBuffer(rawBmp, bmpData, writeBuffer.writeColor8);
    }
    return rawBmp;
};
