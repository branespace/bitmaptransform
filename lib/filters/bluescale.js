...
module.exports = function convert(bmpJSON){
    ...
    if(bmpJSON.hasOwnProperty('colorPalette')){
        for(i = 0; i < bmpJSON.colorPalette.length; i += 1){
            bluescale(bmpJSON.colorPalette[i]); //call bluescale on every color in color palatte
        }
    } else {
...
function bluescale(pixel){ //HAVEN'T WORKED ON THIS YET. NEED TO ONLY CHANGE BLUE VALUES
...
function colorLuminance(color){ //NO CLUE WHAT THIS DOES
...
function imageLuminance(red, green, blue){ //SEEMS LIKE ARBITRARY VALUES?
